...
try:
    ...
    with open("final_tests_output.md", "w", encoding="utf-8") as f:
        ...
        for idx, row in df.iterrows():
            r = row.to_dict()
            res = agent.triage(r)
            results.append({"id": idx+1, "issue": r["issue"], "status": res["status"], "request_type": res["request_type"], "product_area": res.get("product_area", ""), "response": res["response"], "justification": res["justification"]})
